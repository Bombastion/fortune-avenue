...
class BoardControllerTest : DatabaseTest() {
    ...
    private val requiredSpaceTypeSpaces =
        listOf(
     ...
     */
    private fun loopPaths(spaceCount: Int) =
        (0 until spaceCount).map { CreateBoardPathRequest(it, (it + 1) % spaceCount) }

    private fun validRequest(name: String) =
        CreateBoardRequest(
            name = name,
            spaces =
                listOf(
    ...
    fun `creating an invalid board returns 400 with an explanatory message`() {
        ...
        val request =
            validRequest("invalid-${Uuid.random()}")
                .copy(paths = listOf(CreateBoardPathRequest(0, 1)))
                    ...
                    spaces =
                        listOf(
                            CreateBoardSpaceRequest(
                                SpaceType.SHOP,
                                baseValue = 250,
                                basePricePercentage = BigDecimal("0.1500"),
                            ),
                            CreateBoardSpaceRequest(SpaceType.BASIC),
                            CreateBoardSpaceRequest(SpaceType.BASIC),
                        ) + requiredSpaceTypeSpaces,
    ...
    fun `creating a board with a SHOP space missing required fields returns 400`() {
        val request =
            validRequest("shop-invalid-${Uuid.random()}")
                .copy(
                    spaces =
                        listOf(
    ...
    fun `creating a board with district progressions returns them as JSON`() {
        val request =
            validRequest("district-progression-${Uuid.random()}")
                .copy(
                    districts =
                        listOf(
    ...
    fun `creating a board with a space referencing an out-of-range districtIndex returns 400`() {
        val request =
            validRequest("district-oor-${Uuid.random()}")
                .copy(
                    spaces =
                        listOf(
                            CreateBoardSpaceRequest(SpaceType.BASIC, districtIndex = 0),
    ...
    private fun listBoardsPage(query: String): ResponseEntity<Page<BoardResponse>> =
    ...
    private fun createBoards(names: List<String>) =
