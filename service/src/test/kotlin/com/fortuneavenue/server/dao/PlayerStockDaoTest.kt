...
class PlayerStockDaoTest : DatabaseTest() {
	...
	/** A seeded game_district_information row -- player_stocks has a real FK to it, so tests need an actual one, not just any UUID. */
	private fun createGameDistrictInformation(): Uuid {
		...
		val seededDistrictInfo = gameDistrictInformationDao.seedForGame(game.id.value, board, seededShops)
	...
	fun `adjustQuantity adds a positive or negative delta to an existing row's quantity`() {
		...
		val afterSale = playerStockDao.adjustQuantity(player.id.value, gameDistrictInformationId, -5)

		assertThat(afterSale.quantity).isEqualTo(15)
		assertThat(playerStockDao.find(player.id.value, gameDistrictInformationId)!!.quantity).isEqualTo(15)
	}
	...
	fun `findByPlayer returns every district a player has traded stock in`() {
		...
		assertThat(holdings.map { it.gameDistrictInformationId.value }).containsExactlyInAnyOrder(firstDistrictInfoId, secondDistrictInfoId)
	}
