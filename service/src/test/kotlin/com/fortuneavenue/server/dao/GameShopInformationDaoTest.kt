...
class GameShopInformationDaoTest : DatabaseTest() {
    ...
    /**
     * A board with a district containing 2 SHOP spaces, plus a third SHOP space outside any
     * district.
     */
    ...
    fun `findByGameAndSpace finds the seeded row for a space, and null for a space with no shop`() {
        ...
        assertThat(gameShopInformationDao.findByGameAndSpace(gameId, shopSpaceId)?.baseValue)
            .isEqualTo(200)
    ...
    fun `findOwnedByPlayerInDistrict returns only the given player's shops in that district`() {
        ...
        val owned =
            gameShopInformationDao.findOwnedByPlayerInDistrict(gameId, player.id.value, districtId)
    ...
    fun `findByGameAndDistrict returns every shop in that district regardless of owner`() {
        ...
        assertThat(found.map { it.id.value })
            .containsExactlyInAnyOrderElementsOf(inDistrictShops.map { it.id.value })
    }
}
