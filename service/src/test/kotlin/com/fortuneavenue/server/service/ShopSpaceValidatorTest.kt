...
class ShopSpaceValidatorTest {
	...
	private fun shopSpace(baseValue: Int? = 100, basePricePercentage: BigDecimal? = BigDecimal("0.1000")) =
		CreateBoardSpaceRequest(spaceType = SpaceType.SHOP, baseValue = baseValue, basePricePercentage = basePricePercentage)

	private fun basicSpace(baseValue: Int? = null, basePricePercentage: BigDecimal? = null) =
		CreateBoardSpaceRequest(spaceType = SpaceType.BASIC, baseValue = baseValue, basePricePercentage = basePricePercentage)
	...
	fun `a SHOP space with basePricePercentage outside (0, 1) is rejected`() {
		assertThat(ShopSpaceValidator.validate(listOf(shopSpace(basePricePercentage = BigDecimal("0.0000"))))).isNotEmpty()
		assertThat(ShopSpaceValidator.validate(listOf(shopSpace(basePricePercentage = BigDecimal("1.0000"))))).isNotEmpty()
	}

	@Test
	fun `a SHOP space with basePricePercentage that does not have exactly 4 digits is rejected`() {
		assertThat(ShopSpaceValidator.validate(listOf(shopSpace(basePricePercentage = BigDecimal("0.1"))))).isNotEmpty()
		assertThat(ShopSpaceValidator.validate(listOf(shopSpace(basePricePercentage = BigDecimal("0.12345"))))).isNotEmpty()
	}
	...
	fun `a non-SHOP space that includes basePricePercentage is rejected`() {
		val errors = ShopSpaceValidator.validate(listOf(basicSpace(basePricePercentage = BigDecimal("0.1000"))))
