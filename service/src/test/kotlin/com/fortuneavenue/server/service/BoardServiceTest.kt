...
class BoardServiceTest {
    ...
    fun `a valid board is validated then persisted via the DAO`() {
        ...
        val expectedPathInputs =
            request.paths.map { BoardDao.PathInput(it.from, it.to, it.branchOrder) }
    ...
    fun `a board with a valid SHOP space is validated then persisted via the DAO`() {
        val request =
            validRequest().let { req ->
                req.copy(
                    spaces =
                        req.spaces.mapIndexed { index, space ->
                            if (index == 1)
        ...
        val request =
            validRequest().let { req ->
                req.copy(
                    spaces =
                        req.spaces.mapIndexed { index, space ->
                            if (index == 1) CreateBoardSpaceRequest(SpaceType.SHOP) else space
                        }
                )
            }
        ...
        val request =
            validRequest().let { req ->
                req.copy(
                    spaces =
                        req.spaces.mapIndexed { index, space ->
                            if (index == 0) space.copy(baseValue = 100) else space
                        }
                )
            }
    ...
    fun `a board with a valid district is validated then persisted via the DAO`() {
        val request =
            validRequest().let { req ->
                req.copy(
                    ...
                    spaces =
                        req.spaces.mapIndexed { index, space ->
                            if (index == 0) space.copy(districtIndex = 0) else space
                        },
                )
            }
        val expectedGraph = mock(BoardGraph::class.java)
        ...
        val expectedPathInputs =
            request.paths.map { BoardDao.PathInput(it.from, it.to, it.branchOrder) }
        val expectedDistrictInputs =
            request.districts.map {
                BoardDao.DistrictInput(it.name, it.colorHex, it.minimumStockPercentage)
            }
    ...
    fun `a district with a malformed colorHex is rejected without ever touching the DAO`() {
        val request =
            validRequest()
                .copy(
                    districts = listOf(CreateDistrictRequest("Red", "nope", BigDecimal("0.5000")))
                )
                    ...
                    districts =
                        listOf(
                            CreateDistrictRequest(
                                ...
                                progressions = listOf(progression),
                            )
                        ),
                    spaces =
                        req.spaces.mapIndexed { index, space ->
                            if (index <= 1) space.copy(districtIndex = 0) else space
                        },
                )
            }
        val expectedGraph = mock(BoardGraph::class.java)
    ...
    fun `a zero or negative startingGold is rejected without ever touching the DAO`() {
        assertThat(boardService.createBoard(validRequest().copy(startingGold = 0)).isFailure)
            .isTrue()
        assertThat(boardService.createBoard(validRequest().copy(startingGold = -1)).isFailure)
            .isTrue()
        verifyNoInteractions(boardDao)
    }
    ...
    fun `a zero or negative baseSalary is rejected without ever touching the DAO`() {
        ...
        assertThat(boardService.createBoard(validRequest().copy(baseSalary = -1)).isFailure)
            .isTrue()
        verifyNoInteractions(boardDao)
    }
    ...
    fun `a negative promotionBonus is rejected without ever touching the DAO`() {
        assertThat(boardService.createBoard(validRequest().copy(promotionBonus = -1)).isFailure)
            .isTrue()
        verifyNoInteractions(boardDao)
    }
