...
class DistrictValidatorTest {
	...
	fun `a district with a malformed colorHex is rejected`() {
		val req = request(districts = listOf(CreateDistrictRequest("Red", "not-a-color", BigDecimal("0.5000"))))
	...
	fun `a colorHex shorter or longer than 6 characters is rejected`() {
		assertThat(DistrictValidator.validate(request(districts = listOf(CreateDistrictRequest("Red", "FF00", BigDecimal("0.5000")))))).isNotEmpty()
		assertThat(DistrictValidator.validate(request(districts = listOf(CreateDistrictRequest("Red", "FF00000", BigDecimal("0.5000")))))).isNotEmpty()
	}

	@Test
	fun `a lowercase colorHex is accepted`() {
		val req = request(districts = listOf(CreateDistrictRequest("Red", "ff00aa", BigDecimal("0.5000"))))
	...
	fun `a zero or negative minimumStockPercentage is rejected`() {
		assertThat(
			DistrictValidator.validate(request(districts = listOf(CreateDistrictRequest("Red", "FF0000", BigDecimal("0.0000"))))),
		).isNotEmpty()
		assertThat(
			DistrictValidator.validate(request(districts = listOf(CreateDistrictRequest("Red", "FF0000", BigDecimal("-0.5000"))))),
		).isNotEmpty()
	}
	...
	fun `a minimumStockPercentage without exactly 4 digits is rejected`() {
		val req = request(districts = listOf(CreateDistrictRequest("Red", "FF0000", BigDecimal("0.5"))))
