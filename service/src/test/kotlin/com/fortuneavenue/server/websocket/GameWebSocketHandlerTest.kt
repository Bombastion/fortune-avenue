...
class GameWebSocketHandlerTest : DatabaseTest() {
    ...
    @Autowired lateinit var dice: Dice
     ...
     *   roll_dice call (including any consumed by an auto-played computer turn), and pop them off
     *   in order. The Spring context (and so this bean) is shared across every test in the class,
     *   so [resetDice] clears any of the previous test's leftovers before each one runs.
     */
    private class QueuedDice : Dice {
        ...
        override fun roll(): Int =
            queue.poll()
                ?: error(
                    "QueuedDice ran out of queued rolls -- enqueue one for every roll this scenario triggers."
                )
    }

    @TestConfiguration
    class DiceTestConfig {
        @Bean @Primary fun dice(): Dice = QueuedDice()
    }

    /**
     * A tiny WS test client: queues every received event and remembers how the connection closed,
     * if it did.
     */
            ...
            session =
                StandardWebSocketClient()
                    .execute(
                        ...
                        URI(
                            "ws://localhost:$WS_TEST_PORT/ws/game?gameId=$gameId&playerId=$playerId"
                        ),
                    )
        ...
        restTemplate
            .postForEntity<BoardResponse>(
                ...
                CreateBoardRequest(
                    ...
                    // have (see
                    // RequiredSpaceTypesValidator) -- appended after the plain BASIC loop this test
                    // actually
    ...
    /**
     * Space 0 (the start) forks to spaces 1 and 2, each of which leads straight back to 0 -- so
     * movement always pauses for a choice the instant it reaches (or returns to) the start,
     * regardless of the roll.
     */
    private fun createBranchingBoard(): BoardResponse =
        restTemplate
            .postForEntity<BoardResponse>(
                "/boards",
                CreateBoardRequest(
                    ...
                    // Spaces 3-7 are the required BANK + one-of-each-suit spaces every board must
                    // have (see
                    // RequiredSpaceTypesValidator) -- strung along space 1's return leg back to the
                    // start
                    // instead of a direct 1 -> 0 edge, so the fork at space 0 still has exactly the
                    // 2 options
    ...
    private fun createUser(): UserResponse =
        restTemplate
            .postForEntity<UserResponse>(
                "/users",
                CreateUserRequest(username = "user-${Uuid.random()}"),
            )
            .body!!

    /**
     * Omitting [userId] adds a computer player (no user behind the seat) -- pass a real user's id
     * for a human one. Most of these tests want an actual person driving ready-up/roll-dice over
     * the socket, so they pass one; the computer-player test below deliberately doesn't.
     */
    private fun addPlayer(gameId: String, userId: String? = null): PlayerResponse =
        restTemplate
            .postForEntity<PlayerResponse>(
                "/games/$gameId/players",
                AddPlayerRequest(userId = userId),
            )
            .body!!

    private fun addHumanPlayer(gameId: String): PlayerResponse =
        addPlayer(gameId, userId = createUser().id)

    /**
     * A loop of two spaces: the start, and a single SHOP with only one path out of either -- so a
     * roll of 1 from the start always lands on the shop, with nothing to branch on.
     */
    private fun createShopBoard(baseValue: Int = 300, startingGold: Int = 1000): BoardResponse =
        restTemplate
            .postForEntity<BoardResponse>(
                "/boards",
                CreateBoardRequest(
                    name = "shop-${Uuid.random()}",
                    // Spaces 2-6 are the required BANK + one-of-each-suit spaces every board must
                    // have (see
                    // RequiredSpaceTypesValidator) -- strung along the shop's return leg back to
                    // the start so
                    // a roll of 1 from space 0 still lands directly on the SHOP at space 1, as this
                    // test relies on.
                    spaces =
                        listOf(
                            CreateBoardSpaceRequest(SpaceType.BASIC),
     ...
     */
    private fun createDistrictShopBoard(): BoardResponse =
        restTemplate
            .postForEntity<BoardResponse>(
                "/boards",
                CreateBoardRequest(
                    ...
                    // Spaces 3-7 are the required BANK + one-of-each-suit spaces every board must
                    // have (see
                    // RequiredSpaceTypesValidator) -- strung along the second shop's return leg
                    // back to the
                    // start so the two rolls of 1 this test relies on still land on space 1 then
                    // space 2.
                    spaces =
                        listOf(
                            CreateBoardSpaceRequest(SpaceType.BASIC),
    ...
    fun `two players readying up starts the game, and the first player in turn order can roll and move`() {
        ...
        clientA.send("ready")
        // Both clients see A ready up (order between the two isn't guaranteed, so just consume one
        // each).
    ...
    fun `landing on an unowned shop pauses for a purchase, and buy_shop buys it`() {
        ...
        assertThat(client.nextEvent()["type"].asText()).isEqualTo("player_ready")
        assertThat(client.nextEvent()["type"].asText()).isEqualTo("game_started")
        // Sole player and human, so nothing else needs to happen before it's announced as their
        // turn.
    ...
    fun `buying a second shop in the same district recalculates every shop the player owns there`() {
        ...
        // Turn 1: land on and buy the first shop -- only one shop owned yet, nothing to
        // recalculate.
        (dice as QueuedDice).enqueue(1)
        client.send("roll_dice")
        client.nextEvent() // dice_rolled
        ...
        // Turn 2: land on and buy the second shop -- now owning 2 in the district, this
        // recalculates both.
        (dice as QueuedDice).enqueue(1)
        client.send("roll_dice")
        client.nextEvent() // dice_rolled
