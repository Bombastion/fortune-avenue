...
/**
 * Validates a proposed board's graph shape before anything is persisted. A board is valid when:
...
object BoardGraphValidator {
    ...
    fun validate(
        ...
        val outOfRangeEdges = edges.filter {
            it.from !in 0 until spaceCount || it.to !in 0 until spaceCount
        }
        if (outOfRangeEdges.isNotEmpty()) {
            return listOf(
                "Path(s) $outOfRangeEdges reference a space index outside the valid range."
            )
        }
        ...
        val reachableFromStart =
            GraphTraversal.reachableFrom(start, strategy) { forwardNeighbors[it].orEmpty() }
        ...
        val canReachStart =
            GraphTraversal.reachableFrom(start, strategy) { backwardNeighbors[it].orEmpty() }
