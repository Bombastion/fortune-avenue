...
@Service
class BoardService(private val boardDao: BoardDao) {
    ...
    fun createBoard(request: CreateBoardRequest): Result<BoardGraph> {
        ...
        val startingGoldErrors =
            if (request.startingGold > 0) emptyList()
            else listOf("startingGold must be a positive integer.")
        val baseSalaryErrors =
            if (request.baseSalary > 0) emptyList()
            else listOf("baseSalary must be a positive integer.")
        val promotionBonusErrors =
            if (request.promotionBonus >= 0) emptyList()
            else listOf("promotionBonus must be zero or a positive integer.")

        val errors =
            ShopSpaceValidator.validate(request.spaces) +
