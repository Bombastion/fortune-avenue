...
object DistrictProgressionValidator {
	...
	fun validate(request: CreateBoardRequest): List<String> {
		val spaceCountByDistrictIndex = request.spaces
			.mapNotNull { it.districtIndex }
			.groupingBy { it }
			.eachCount()

		return request.districts.flatMapIndexed { index, district ->
			val spaceCount = spaceCountByDistrictIndex[index] ?: 0
			val requiredLevels = if (spaceCount >= MIN_SPACES_REQUIRING_PROGRESSIONS) {
	...
	private fun levelErrors(index: Int, spaceCount: Int, requiredLevels: Set<Int>, actualLevels: List<Int>): List<String> {
		val errors = mutableListOf<String>()

		val duplicates = actualLevels.groupingBy { it }.eachCount().filterValues { it > 1 }.keys.sorted()
		if (duplicates.isNotEmpty()) {
			errors += "District at index $index defines duplicate progression entries for ownedShopCount $duplicates."
		}
		...
		if (missing.isNotEmpty() || extra.isNotEmpty()) {
			...
			errors += "District at index $index has $spaceCount space(s) and must define exactly one progression entry for each " +
	...
	private fun percentageErrors(index: Int, progression: CreateDistrictProgressionRequest): List<String> {
		val errors = mutableListOf<String>()
