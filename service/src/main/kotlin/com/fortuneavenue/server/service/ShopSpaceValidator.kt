...
/**
 * Validates the SHOP-specific fields on a board creation request: SHOP spaces must carry a positive
 * [CreateBoardSpaceRequest.baseValue] and a [CreateBoardSpaceRequest.basePricePercentage] strictly
 * between 0 and 1 with exactly 4 digits; every other space type must omit both.
 */
object ShopSpaceValidator {

    fun validate(spaces: List<CreateBoardSpaceRequest>): List<String> =
        spaces.mapIndexedNotNull { index, space ->
    ...
    private fun shopSpaceError(index: Int, space: CreateBoardSpaceRequest): String? {
        ...
        return if (problems.isEmpty()) null
        else
            "Space at index $index is a SHOP space and must have ${problems.joinToString(" and ")}."
    }
