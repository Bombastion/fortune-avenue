 ...
 * `buy_stock` and `sell_stock` need a districtId and a quantity (1-99):
 * `{"type": "buy_stock", "districtId": "<id>", "quantity": 10}` -- which
 * BANK stop they resolve is implied the same way, by whichever
 * `stock_trading_available` event is currently pending. `skip_stock_trade`
 * needs nothing extra, same as `decline_shop`.
 */
