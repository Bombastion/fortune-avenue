...
class BoardDao {
    ...
    fun create(
        ...
        val districtProgressions =
            districtInputs.zip(districts).flatMap { (input, district) ->
        ...
        val shopInformation =
            spaceInputs.zip(spaces).mapNotNull { (input, space) ->
                ...
                ShopInformation.new {
                    ...
                    spaceId = space.id
                    baseValue =
                        requireNotNull(input.baseValue) {
                            "SHOP space at ${space.id.value} is missing baseValue."
                        }
                    basePricePercentage =
                        requireNotNull(input.basePricePercentage) {
    ...
    private fun districtProgressionsFor(districts: List<District>): List<DistrictValueProgression> {
        ...
        return DistrictValueProgression.find {
                DistrictValueProgressionsTable.districtId inList districtIds
            }
            .toList()
    }

    /**
     * A single-column lookup for [com.fortuneavenue.server.service.PlayerService] -- cheaper than
     * loading a full [BoardGraph] via [findById].
     */
    fun findStartingGold(id: Uuid): Int? = transaction { Board.findById(id)?.startingGold }

    /**
     * The progression row for a district at a specific owned-shop count, if one's been defined --
     * see [GameSimulationService][com.fortuneavenue.server.service.GameSimulationService]'s shop
     * purchase handling.
     */
    fun findDistrictValueProgression(
        districtId: EntityID<Uuid>,
        ownedShopCount: Int,
    ): DistrictValueProgression? = transaction {
    ...
    fun findById(id: Uuid): BoardGraph? = transaction {
        ...
        val shopInformation =
            ShopInformation.find { ShopInformationTable.boardId eq board.id }.toList()
    ...
    fun findPage(page: Int, pageSize: Int, ascending: Boolean = true): List<BoardGraph> =
        transaction {
    ...
    /**
     * Total number of boards, regardless of any page/pageSize -- used to compute how many pages
