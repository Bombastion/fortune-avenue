...
class PlayerStockDao {
    ...
    /**
     * Every district [playerId] currently holds any stock in (or has ever traded), across their one
     * game.
     */
    ...
    /**
     * Adds [delta] (negative to sell) to [playerId]'s held quantity of
     * [gameDistrictInformationId]'s stock, creating the row (starting from 0) the first time this
     * player trades that district's stock
     */
    fun adjustQuantity(playerId: Uuid, gameDistrictInformationId: Uuid, delta: Int): PlayerStock =
        transaction {
            ...
            if (existing != null) {
                ...
                PlayerStock.new {
                    ...
                    this.gameDistrictInformationId =
                        EntityID(gameDistrictInformationId, GameDistrictInformationTable)
                    this.quantity = delta
                }
            }
        }
    ...
    private fun findEntity(playerId: Uuid, gameDistrictInformationId: Uuid): PlayerStock? =
        PlayerStock.find {
                (PlayerStocksTable.playerId eq EntityID(playerId, PlayersTable)) and
                    (PlayerStocksTable.gameDistrictInformationId eq
                        EntityID(gameDistrictInformationId, GameDistrictInformationTable))
            }
            .firstOrNull()
}
