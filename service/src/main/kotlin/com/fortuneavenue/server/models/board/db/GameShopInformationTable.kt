...
/**
 * Per-game copy of a [ShopInformation] row. Board templates are reusable across games, so a shop's
 * mutable in-game state -- current value, owner, investment -- lives here instead, one row per
 * (game, shop), seeded from shop_information when a game starts.
 *
...
object GameShopInformationTable : UuidTable("game_shop_information") {
    ...
    val basePricePercentage =
        decimal(
            "base_price_percentage",
            BASE_PRICE_PERCENTAGE_PRECISION,
            BASE_PRICE_PERCENTAGE_SCALE,
        )
