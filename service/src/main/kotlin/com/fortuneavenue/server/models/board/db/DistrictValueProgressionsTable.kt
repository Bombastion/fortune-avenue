 ...
 * level. Deliberately per-district (not a shared board-level curve): same-sized districts can still
 * scale differently from each other if board makers desire.
 */
object DistrictValueProgressionsTable : UuidTable("district_value_progressions") {
    ...
    val existingShopBoostPercentage =
        decimal(
            "existing_shop_boost_percentage",
            BOOST_PERCENTAGE_PRECISION,
            BOOST_PERCENTAGE_SCALE,
        )
    ...
    val newShopBoostPercentage =
        decimal("new_shop_boost_percentage", BOOST_PERCENTAGE_PRECISION, BOOST_PERCENTAGE_SCALE)
