...
/**
 * Nested under /games/{gameId} rather than a top-level /players -- a player only ever makes sense
...
class PlayerController(private val playerService: PlayerService) {
    ...
    fun addPlayer(
            ...
            request.userId?.let {
                Uuid.parseOrNull(it)
                    ?: return ResponseEntity.badRequest()
                        .body<Any>(ErrorResponse("userId is not a valid id."))
            }
        ...
        return result.fold(
            onSuccess = { player ->
                ResponseEntity.status(HttpStatus.CREATED).body<Any>(player.toResponse())
            },
            onFailure = { error ->
                when (error) {
                    is GameNotFoundException -> ResponseEntity.notFound().build()
                        ...
                        ResponseEntity.badRequest()
                            .body<Any>(ErrorResponse(error.message ?: "Invalid player"))
                }
            },
        )
    }
    ...
    fun getPlayers(@PathVariable gameId: String): ResponseEntity<List<PlayerResponse>> {
        ...
        val players =
            playerService.getPlayers(parsedGameId) ?: return ResponseEntity.notFound().build()
