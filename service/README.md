...
- **REST** handles everything that isn't part of a live game session: health checks (`/health`), users (`/users`), boards (`/boards`), games (`/games`, tied to a board), and players (`/games/{gameId}/players`, optionally tied to a user) today, with room for auth, lobbies, and history later. Validation lives in the `Service` layer, not controllers, so it's consistent no matter what calls it.
